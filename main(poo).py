...
#CRIAÇÃO DO OBJETO NAVE DO BYTE
nave = Nave(screen_altura=altura, screen_largura=largura, speed=4, delay=200)
naves_group = pygame.sprite.GroupSingle()
naves_group.add(nave)

#CRIAÇÃO DO GRUPO DE QUE CONTEM OS OBJETOS INSTANCIADOS DA CLASSE ENEMY
...
sprite_inimigo = pygame.image.load('C:/Users/mique/byte_in_space/imagens/inimigo0.png')
...
sprite_item = pygame.image.load('C:/Users/mique/byte_in_space/imagens/computador.png')
...
#VARIAVEIS DE CONTROLE DE SPAWN DOS ENEMIES
tempo_ultimo_inimigo = pygame.time.get_ticks()  #tempo do último inimigo criado (começa agora)
intervalo_spawn_inimigo = 500 #tempo entre um inimigo e outro (em milissegundos) — 1500ms = 1,5 segundos
    ...
    #                    itens.append([inimigo[0], inimigo[1]])
     ...
     #           break
        ...
        if pygame.time.get_ticks() - tempo_ultimo_inimigo >= intervalo_spawn_inimigo:
            enemy = Enemy(0, tuple(criar_inimigo()), -1, altura//2)
        ...
        #UPDATING
        colisoes_enemy_bullet = pygame.sprite.groupcollide(enemies_group, naves_group.sprite.bullets_group, True, True)
        colisao_nave_enemy = pygame.sprite.spritecollide(nave, enemies_group, False)
        if len(colisao_nave_enemy) != 0:
            game_over = True
            tempo_game_over = pygame.time.get_ticks()
            nave.kill()
            for colisao in colisao_nave_enemy:
                colisao.kill()
        else:
            naves_group.update()
            ...
            naves_group.sprite.bullets_group.draw(tela)
